...
router.get('/:bucket/:id', async function (req, res, next) {
    ...
    try {
        ...
        nextWebhook = await webhookModel.find({ _id: { $gt: results._id } }).sort({ _id: 1 }).limit(1);
        previousWebhook = await webhookModel.find({ _id: { $lt: results._id } }).sort({ _id: 1 }).limit(1);
    } catch (err) {
        return next(err);
    }

    nextWebhook = nextWebhook.map(w => w._id)[0];
    previousWebhook = previousWebhook.map(w => w._id)[0];

    results.nextId = nextWebhook.toString();
    results.previousId = previousWebhook.toString();
    return res.json({ webhook: results, nextWebhook, previousWebhook });
});
